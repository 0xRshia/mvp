...
- Discovery groups Free → Suggested → New → All in RTL swipeable rows, with eight-event previews and a final listing link. `/events` and `/events/free` show full filtered grids; `?view=new` orders known creation dates newest first, and `?view=suggested` preserves recommendation ranking with an eight-event cap. Browsing filters survive client navigation in memory; reloading restores defaults and never persists precise coordinates. Free cards omit countdowns without changing registration deadlines.
...
- Event details, finite or unlimited capacity, 1–6 tickets per order, free registration, paid holds and redirect to Zarinpal. Available-capacity badges use teal with white text; sold-out badges remain neutral. Paid event details show an animated Persian hours/minutes/seconds countdown below the title within the same 48-hour window as cards. Reduced motion disables digit animations, not time updates; booking deadlines remain authoritative.
- Iranian phone normalization, expiring SMS OTPs, rate limits, single-use verification, database-backed sessions and logout.
- Current and historical reservations, payment retries/rechecks, and a PDF download immediately after confirmation and from purchase history. Each purchased seat has a separate Persian ticket page with event address and its own persistent QR. Future unused free reservations can be cancelled.
