...
- Jalali input and display, Tehran timezone, Persian digits, Toman prices, responsive client navigation and self-hosted Iran Sans X. Critical WOFF2 weights are preloaded with optional font display to avoid late font swaps. Pages and tab panels enter over 800ms with top-to-bottom starts spread across a maximum 550ms window. Controls acknowledge activation over 600ms while actions run immediately. Reduced motion disables movement and stagger without delaying updates. The event-detail countdown uses dark surfaces in dark mode.
