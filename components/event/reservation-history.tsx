  ...
  return (
    <main data-motion-group className={`container subpage ${layouts.page} ${layouts.reservationsPage}`}>
      <div className={`page-heading ${layouts.pageHeading}`}>
        ...
        <h1>{account ? "حساب کاربری" : "بلیت‌های من"}</h1>
        <p>همهٔ قرارهای پیش رو و خاطره‌های قبلی، یک‌جا.</p>
      </div>
      {account && user && (
        <section className={layouts.accountProfile} aria-label="اطلاعات حساب">
          <div className={layouts.accountIdentity}>
            <span className={layouts.accountAvatar}><UserRound size={28} aria-hidden="true" /></span>
            <div>
              <h2>{user.name || "حساب شما"}</h2>
              <p>شمارهٔ همراه <bdi>{faDigits(user.phone)}</bdi></p>
            </div>
          </div>
          <button className="button outline" type="button" onClick={logout} disabled={loggingOut} aria-busy={loggingOut}>
            <LogOut size={17} aria-hidden="true" />
            <ButtonLabel busy={loggingOut} pending="در حال خروج…">خروج از حساب</ButtonLabel>
          </button>
        </section>
      )}
      {account && user && <h2 className={layouts.accountHistoryTitle}>بلیت‌ها و قرارهای من</h2>}
